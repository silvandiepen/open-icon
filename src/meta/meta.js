...
async function processIcons() {
    ...
    for (const iconName of AllIcons) {
        ...
        if (fs.existsSync(metaFilePath)) {
            ...
            const existingMeta = JSON.parse(fs.readFileSync(metaFilePath.replace('.ts', '.json'), 'utf-8'));
    ...
    ${AllIcons.map((icon) => `import ${upperSnakeCase(icon)}_META from './${icon.toLowerCase()}';`).join('\n')}
