...
async function generateIconMeta(iconName: string): Promise<IconMeta> {
...
async function processIcons() {
    let globalCategories = new Set<string>();
    ...
    ${AllIcons.map((icon: string) => `import * as ${upperSnakeCase(icon)}_META from './${icon.toLowerCase()}';`).join('\n')}
